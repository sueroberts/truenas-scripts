...
get_smart_drives()
{
  ...
  for gs_drive in $gs_drives; do
    gs_smart_flag=$("$smartctl" -i "$gs_drive" | grep -E "SMART support is:[[:blank:]]+Enable>
...
get_sata_drives()
{
  for drive in $Drive_list; do
    lFound=0
    gsata_smart_flag=$("$smartctl" -i "$drive" | grep -E "SATA Version is:[[:blank:]]" | awk >
    if [ "$gsata_smart_flag" = "SATA" ]; then
      lFound=$((lFound + 1))
    else
      gsata_smart_flag=$("$smartctl" -i "$drive" | grep -E "ATA Version is:[[:blank:]]" | awk>
...
{
  for drive in $Drive_list; do
    gsas_smart_flag=$("$smartctl" -i "$drive" | grep -E "Transport protocol:[[:blank:]]+SAS" >
...
if [ $SATA_count -gt 0 ]; then
  (
   ...
   echo ""
   echo "+------+------------------------+----+------+-----+-----+-------+-------+--------+-->
   echo "|Device|Serial                  |Temp| Power|Start|Spin |ReAlloc|Current|Offline |Se>
   echo "|      |Number                  |    | On   |Stop |Retry|Sectors|Pending|Uncorrec|Er>
   echo "|      |                        |    | Hours|Count|Count|       |Sectors|Sectors |  >
   echo "+------+------------------------+----+------+-----+-----+-------+-------+--------+-->
  ) >> "$logfile"
  ...
  for drive in $SATA_list; do
    ...
    awk -v device="$devid" -v tempWarn="$tempWarn" -v tempCrit="$tempCrit" -v sectorsCrit="$s>
    ...
    END {
      ...
      if (temp > tempCrit || reAlloc > sectorsCrit || pending > sectorsCrit || offlineUnc > s>
        device=device " " critSymbol;
      else if (temp > tempWarn || reAlloc > 0 || pending > 0 || offlineUnc > 0 || testAge > t>
...
if [ $SAS_count -gt 0 ]; then
  (
    ...
    echo "|      |Number                  |    |Stop |Unload|List  |Read  |Write |Verify|Medi>
    echo "|      |                        |    |Count|Count |Elems |Errors|Errors|Errors|Erro>
    echo "+------+------------------------+----+-----+------+------+------+------+------+---->
  ) >> "$logfile"
...
if [ $SATA_count -gt 0 ] || [ $SAS_count -gt 0 ]; then
  ...
  for drive in $SATA_list; do
    ...
    if [ -z "$vendor" ]; then
      dfamily=$("$smartctl" -i "$drive" | grep "Model Family" | awk '{print $3, $4, $5, $6, $>
      dmodel=$("$smartctl" -i "$drive" | grep "Device Model" | awk '{print $3, $4, $5, $6, $7>
      if [ -z "$dfamily" ]; then
        dinfo=$dmodel
      else
